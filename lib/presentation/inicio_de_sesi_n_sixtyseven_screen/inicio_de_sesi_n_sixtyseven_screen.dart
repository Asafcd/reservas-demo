import 'bloc/inicio_de_sesi_n_sixtyseven_bloc.dart';
import 'models/inicio_de_sesi_n_sixtyseven_model.dart';
...
class InicioDeSesiNSixtysevenScreen extends StatelessWidget {
  const InicioDeSesiNSixtysevenScreen({Key? key})
      : super(
          key: key,
        );

  static Widget builder(BuildContext context) {
    return BlocProvider<InicioDeSesiNSixtysevenBloc>(
      create: (context) =>
          InicioDeSesiNSixtysevenBloc(InicioDeSesiNSixtysevenState(
        inicioDeSesiNSixtysevenModelObj: InicioDeSesiNSixtysevenModel(),
      ))
            ..add(InicioDeSesiNSixtysevenInitialEvent()),
      child: InicioDeSesiNSixtysevenScreen(),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<InicioDeSesiNSixtysevenBloc,
                    ...
                    Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          SizedBox(height: 14.v),
                          _buildInvitadoCounterJenniferHernandez(context),
                          SizedBox(height: 14.v),
                          _buildInvitadoCounterDavidInurretaGomez(context),
  ...
  Widget _buildInvitadoCounterCarolinaInurreta(BuildContext context) {
    return BlocSelector<InicioDeSesiNSixtysevenBloc,
        InicioDeSesiNSixtysevenState, TextEditingController?>(
  ...
  /// Section Widget
  Widget _buildInvitadoCounterJenniferHernandez(BuildContext context) {
    return BlocSelector<InicioDeSesiNSixtysevenBloc,
        InicioDeSesiNSixtysevenState, TextEditingController?>(
      selector: (state) => state.invitadoCounterJenniferHernandezController,
      builder: (context, invitadoCounterJenniferHernandezController) {
        return CustomFloatingTextField(
          controller: invitadoCounterJenniferHernandezController,
  ...
  /// Section Widget
  Widget _buildInvitadoCounterDavidInurretaGomez(BuildContext context) {
    return BlocSelector<InicioDeSesiNSixtysevenBloc,
        InicioDeSesiNSixtysevenState, TextEditingController?>(
      selector: (state) => state.invitadoCounterDavidInurretaGomezController,
      builder: (context, invitadoCounterDavidInurretaGomezController) {
        return CustomFloatingTextField(
          controller: invitadoCounterDavidInurretaGomezController,
