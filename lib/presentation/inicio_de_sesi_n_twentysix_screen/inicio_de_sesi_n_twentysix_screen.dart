import '../inicio_de_sesi_n_twentysix_screen/widgets/horarios_item_widget.dart';
import '../inicio_de_sesi_n_twentysix_screen/widgets/segmentedbutton_item_widget.dart';
import 'bloc/inicio_de_sesi_n_twentysix_bloc.dart';
import 'models/horarios_item_model.dart';
import 'models/inicio_de_sesi_n_twentysix_model.dart';
...
class InicioDeSesiNTwentysixScreen extends StatelessWidget {
  InicioDeSesiNTwentysixScreen({Key? key})
      : super(
          key: key,
        );
  ...
  static Widget builder(BuildContext context) {
    return BlocProvider<InicioDeSesiNTwentysixBloc>(
      create: (context) =>
          InicioDeSesiNTwentysixBloc(InicioDeSesiNTwentysixState(
        inicioDeSesiNTwentysixModelObj: InicioDeSesiNTwentysixModel(),
      ))
            ..add(InicioDeSesiNTwentysixInitialEvent()),
      child: InicioDeSesiNTwentysixScreen(),
    );
  }
  ...
  Widget _buildSegmentedButton(BuildContext context) {
    return BlocSelector<InicioDeSesiNTwentysixBloc, InicioDeSesiNTwentysixState,
        InicioDeSesiNTwentysixModel?>(
      selector: (state) => state.inicioDeSesiNTwentysixModelObj,
      builder: (context, inicioDeSesiNTwentysixModelObj) {
        return Wrap(
          runSpacing: 8.v,
          spacing: 8.h,
          children: List<Widget>.generate(
            inicioDeSesiNTwentysixModelObj?.segmentedbuttonItemList.length ?? 0,
            (index) {
              SegmentedbuttonItemModel model = inicioDeSesiNTwentysixModelObj
                      ?.segmentedbuttonItemList[index] ??
                  SegmentedbuttonItemModel();

              return SegmentedbuttonItemWidget(
                model,
                onSelectedChipView: (value) {
                  context.read<InicioDeSesiNTwentysixBloc>().add(
    ...
    return BlocSelector<InicioDeSesiNTwentysixBloc, InicioDeSesiNTwentysixState,
      ...
      builder: (context, inicioDeSesiNTwentysixModelObj) {
        return Wrap(
          ...
          spacing: 10.h,
          children: List<Widget>.generate(
            inicioDeSesiNTwentysixModelObj?.horariosItemList.length ?? 0,
            (index) {
              HorariosItemModel model =
                  inicioDeSesiNTwentysixModelObj?.horariosItemList[index] ??
                      HorariosItemModel();

              return HorariosItemWidget(
                model,
                onSelectedChipView: (value) {
                  context.read<InicioDeSesiNTwentysixBloc>().add(
